    ...
    public static check(package_json_content:any):boolean{
        ...
        if( next_version.includes("canary") ){
            throw new InvalidNexJsVersion(
                `Azion Framework Adapter does not offers support to nextjs canary versions and requires at least Nextjs v:${Object.values(NextJsVersions).toString()}.
                You are using v:${next_version}. Please try to update your version of Nextjs.`
            );
        }
